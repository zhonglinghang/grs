...
func (e *Encoder) EncodeAmf3StringValue(w io.Writer, str string) (int, error) {
	return e.EncodeAmf3String(w, str, true)
}

func (e *Encoder) EncodeAmf3IntValue(w io.Writer, val int64) (int, error) {
	if val >= 0 && val <= AMF3_INTEGER_MAX {
		return e.EncodeAmf3Integer(w, uint32(val), true)
	} else {
		return e.EncodeAmf3Double(w, float64(val), true)
	}
}

func (e *Encoder) EncodeAmf3ObjectValue(w io.Writer, val Object) (int, error) {
	if val == nil {
		return e.EncodeAmf3Null(w, true)
	}
