...
func (connServer *ConnServer) writeMsgV2(csid, streamID uint32, name string, transactionId int, resp amf.Object, event amf.Object) error {
	connServer.bytesw.Reset()
	if _, err := connServer.encoder.EncodeString(connServer.bytesw, name, amf.AMF0); err != nil {
		return err
	}

	if _, err := connServer.encoder.EncodeInt(connServer.bytesw, transactionId, amf.AMF0); err != nil {
		return err
	}

	if _, err := connServer.encoder.EncodeObject(connServer.bytesw, resp, amf.AMF0); err != nil {
		return err
	}

	if _, err := connServer.encoder.EncodeObject(connServer.bytesw, event, amf.AMF0); err != nil {
		return err
	}
	msg := connServer.bytesw.Bytes()
	c := ChunkStream{
...
func (connServer *ConnServer) writeMsgV2sioi(csid, streamID uint32, name string, transactionId int, resp amf.Object, id int) error {
	connServer.bytesw.Reset()
	if _, err := connServer.encoder.EncodeString(connServer.bytesw, name, amf.AMF0); err != nil {
		return err
	}
	...
	if _, err := connServer.encoder.EncodeInt(connServer.bytesw, id, amf.AMF0); err != nil {
		return err
	}
	msg := connServer.bytesw.Bytes()
	c := ChunkStream{
...
func (connServer *ConnServer) connectResp(cur *ChunkStream) error {
	...
	return connServer.writeMsgV2(cur.CSID, cur.StreamID, "_result", connServer.transactionID, resp, event)
}
...
func (connServer *ConnServer) createStreamResp(cur *ChunkStream) error {
	return connServer.writeMsgV2sioi(cur.CSID, cur.StreamID, "_result", connServer.transactionID, nil, connServer.streamID)
}
...
func (connServer *ConnServer) publishResp(cur *ChunkStream) error {
	...
	return connServer.writeMsgV2(cur.CSID, cur.StreamID, "onStatus", 0, nil, event)
}
...
func (connServer *ConnServer) playResp(cur *ChunkStream) error {
	...
	if err := connServer.writeMsgV2(cur.CSID, cur.StreamID, "onStatus", 0, nil, event); err != nil {
		return err
	}
